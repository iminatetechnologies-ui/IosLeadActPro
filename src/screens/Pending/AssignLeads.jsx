...
const AssignLeads = ({navigation, refreshData, updateAssignedCount}) => {
  ...
  useFocusEffect(
    ...
    }, [isFilterApplied, activeFilterParams,usertype]), // dependencies
  );
  ...
  const fetchData = async (params = {}) => {
    setIsLoading(true);
    try {
      const api = getFreshLeadAPI(); // dynamic API
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      const api = getFreshLeadAPI(); // dynamic API
      console.log('🔵 API Call on refresh:', api, 'Params:', params);
  ...
  //   setIsLoading(true);
  //   try {
  //     const response = await _get('/assignedleads', {params});
  //     const result = response?.data;
  //     const count = response?.data?.leadcount ?? 0;
  //     setLeadcount(count);
  //     if (result) {
      ...
      {isLoading ? (
        ...
        <>
          ...
          <SectionList
            ...
            ListEmptyComponent={
              !isLoading && <Text style={styles.emptyText}>No lead found.</Text>
            }
            ListFooterComponent={
