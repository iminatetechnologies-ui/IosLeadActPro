...
// import FilterModal from '../../components/FilterModalunassign';
...
// const UnAssignLeads = ({navigation, refreshData, updateUnassignedCount}) => {
...
//     setIsLoading(true);
//     try {
//       const api = getUnassignedAPI(); // 🔥 dynamic API

//       const response = await _get(api, {params});
...
//       const leadcount = response?.data?.total_leads || '0';
...
//         if (updateUnassignedCount) {
//           updateUnassignedCount(leadcount);
//         }
//       } else {
...
//   //   setIsLoading(true);
//   //   try {
//   //     const response = await _get('/unassignedleads', {params});
...
//     if (!(usertype === 'company' || usertype === 'team_owner')) return;
...
//       showCustomAlert('Error', 'Unable to open the messaging app.'),
//     );
//   };
...
//     Linking.openURL(url).catch(() =>
//       showCustomAlert('Error', 'WhatsApp is not installed on your device.'),
//     );
//   };
...
//     Linking.openURL(url).catch(() =>
//       showCustomAlert('Error', 'Phone number is not available.'),
//     );
//   };
...
//           substatus_name={item?.status_name}
//           source={item?.source || '-'}
//           team_owner={
...
//           }
//           oncardPress={() => {
//             // ✅ Correct: Selection mode mein ho tabhi toggle karo
//             if (
//               (usertype === 'company' || usertype === 'team_owner') &&
...
//             ListEmptyComponent={
//               !isLoading && <Text style={styles.emptyText}>No Data found.</Text>
//             }
//             ListFooterComponent={
...
const UnAssignLeads = ({navigation, refreshData, updateUnassignedCount}) => {
  ...
  useFocusEffect(
    useCallback(() => {
      // Pehle usertype load hone ka wait karo
      if (usertype) {
        if (isFilterApplied) {
          const params = buildFilterParams();
          fetchData(params);
  ...
  const fetchData = async (params = {}, skipLoading = false) => {
    if (!skipLoading) {
      setIsLoading(true);
    }
    try {
      const api = getUnassignedAPI();
      console.log('🔵 API Call:', api, 'Params:', params);
      ...
      if (result) {
        // API se jo data aaye, waise hi set karo - NO SORTING
        const dataArray = result?.data || [];
        
        // Console log - API response ka exact order
        console.log('✅ API Response Data Count:', dataArray.length);
        console.log('📋 Lead IDs in Order:', dataArray.map(i => i.id));
        console.log('📋 Lead Names in Order:', dataArray.map(i => i.name));
        console.log('🔢 First 5 Items:', dataArray.slice(0, 5));
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      const params = isFilterApplied ? buildFilterParams() : {};
      await fetchData(params, true); // skipLoading = true
    } catch (error) {
      showError('Failed to refresh data.');
    } finally {
      setRefreshing(false);
  ...
  const getAllItemIds = () => {
    // Sirf visible items ke IDs return karo (based on visibleCount)
    return data.slice(0, visibleCount).map(item => item.id);
  };

  const areAllSelected = () => {
    const visibleIds = getAllItemIds();
          ...
          {selectedIds.length > 0 && (
            <View style={styles.selectAllContainer}>
              <TouchableOpacity
                ...
                <Text style={styles.selectAllText}>
                  {areAllSelected() ? 'Unselect All' : `Select All (${Math.min(visibleCount, data.length)})`}
                </Text>
              </TouchableOpacity>
