...
const TodayScreen = ({navigation, userId}) => {
  const [data, setData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [visibleCount, setVisibleCount] = useState(100);

  const apiCalled = useRef(false); // 👈 added to prevent repeated API calls
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await _get(`/todaycallbackleadsbyuser/${userId}`);
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(result)) {
        setData(result);
        setVisibleCount(100); // reset pagination on refresh
      } else {
        Alert.alert('No data found.');
      }
    } catch (error) {
