...
const UpcomingScreen = ({navigation}) => {
  ...
  const categorizeLeadsByDate = leads => {
    ...
    if (laterLeads.length > 0) {
      sections.push({
        title: 'Later',
        data: laterLeads,
        count: laterLeads.length,
      });
    }
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await _get('/getupcomingcallback');
      ...
      {isLoading ? (
        ...
        <>
          ...
          <SectionList
            ...
            contentContainerStyle={styles.list}
            ListEmptyComponent={
              <Text style={styles.emptyText}>No Upcoming leads found.</Text>
            }
            ListFooterComponent={
