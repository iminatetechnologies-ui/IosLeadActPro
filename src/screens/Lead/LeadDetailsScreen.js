...
export default function LeadDetailsScreen({navigation, route}) {
  ...
  const handleInterestedPress = async () => {
    try {
      ...
      // Fetch call logs before navigation
      // await fetchCallLogsForLead(item?.id, leadDetails?.mobile);

      // Navigate with full data instead of partial item
  ...
  const handleCallBackPress = async () => {
    try {
      ...
      navigation.replace('CallBackDetails', {
        item: item,
  ...
  const handleRingingPress = async () => {
    try {
      setStatusSelected(true);

      const res = await _get(`/leads/ringing/${item?.id}`);
      //  console.log('999999999999999999--------', res);

      if (res?.status === 200) {
        Alert.alert('Success', 'Status updated to Ringing successfully!', [
          {
            text: 'OK',
            onPress: () => navigation.goBack(), // ← page se bahar nikal jao
          },
        ]);
      } else {
      ...
      console.log('❌ ERROR in handleRingingPress:', {
  ...
  const handleNotInterestedPress = async () => {
    try {
      ...
      navigation.replace('NotIntrested', {
        item: item,
        callLogs: callLog, // Pass call logs to next screen
      });
    } catch (error) {
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerIcons}>
              {userType !== 'employee' ? (
                ...
                <View style={{width: 35, height: 35}} /> // Placeholder so layout same rahe
              )}
            ...
            <View style={styles.card}>
              ...
              <View style={styles.buttonContainer}>
                ...
                <TouchableOpacity
                  ...
                  onPress={handleRingingPress}>
                  <Text style={styles.buttonText}>Ringing</Text>
                </TouchableOpacity>
                <TouchableOpacity
