...
export default function Leave() {
  ...
  const openPanel = async type => {
    ...
    if (type === 'Pending Request') {
      try {
        ...
        if (usertype === 'company' || usertype === 'team_owner') {
          ...
          response = await _get('/adminrequest?status=0&subject=Leave');
        } else {
          // For normal users, call /userrequests with filters
          response = await _get('/userrequests?status=pending&subject=Leave');
        }
    ...
    if (type === 'Approved Request') {
      try {
        ...
        if (usertype === 'company' || usertype === 'team_owner') {
          ...
          response = await _get('/adminrequest?status=1&subject=Leave');
        } else {
          // For other users, use filtered /userrequests API
          response = await _get('/userrequests?status=approved&subject=Leave');
        }
    ...
    if (type === 'Rejected Request') {
      try {
        ...
        if (usertype === 'company' || usertype === 'team_owner') {
          response = await _get('/adminrequest?status=2&subject=Leave');
        } else {
          response = await _get('/userrequests?status=rejected&subject=Leave');
        }
  ...
  const handleApprove = async requestId => {
    try {
      ...
      const response = await _get('/userrequests?status=pending&subject=Leave');
      const requestsData = response?.data?.data;
      setPendingRequests(Array.isArray(requestsData) ? requestsData : []);
    } catch (error) {
      console.error(
