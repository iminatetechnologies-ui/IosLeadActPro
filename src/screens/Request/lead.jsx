...
export default function Lead() {
  ...
  const openPanel = async type => {
    ...
    if (type === 'Pending Request') {
      try {
        ...
        if (usertype === 'company' || usertype === 'team_owner') {
          ...
          response = await _get('/adminrequest?status=0&subject=Lead');
        } else {
          // For normal users, call /userrequests with filters
          response = await _get('/userrequests?status=pending&subject=Lead');
        }
    ...
    if (type === 'Approved Request') {
      try {
        ...
        if (usertype === 'company' || usertype === 'team_owner') {
          ...
          response = await _get('/adminrequest?status=1&subject=Lead');
        } else {
          // For other users, use filtered /userrequests API
          response = await _get('/userrequests?status=approved&subject=Lead');
        }
    ...
    if (type === 'Rejected Request') {
      try {
        ...
        if (usertype === 'company' || usertype === 'team_owner') {
          response = await _get('/adminrequest?status=2&subject=Lead');
        } else {
          response = await _get('/userrequests?status=rejected&subject=Lead');
        }
  ...
  const handleApprove = async requestId => {
    try {
      ...
      const response = await _get('/userrequests?status=pending&subject=Lead');
      const requestsData = response?.data?.data;
      setPendingRequests(Array.isArray(requestsData) ? requestsData : []);
    } catch (error) {
      console.error(
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.leftPanel}>
        <Text style={styles.heading}>Lead Requests</Text>
