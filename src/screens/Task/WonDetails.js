...
//     let newErrors = {};
//     setSuccessMessage(''); // clear old msg
...
//     setIsLoading(true);
//     try {
//       const response = await _post(`/leads/wonleads/save/${item?.id}`, data);
...
const handleSubmit = async () => {
  ...
  // -------------------------------
  // 🔥 NEW CONDITION:
  // Notes OR Audio → koi ek must
  // -------------------------------
  if (!notes.trim() && !recordedFile) {
    newErrors.main = 'Please enter Notes or record Audio (one is required)';
  }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        {/* ❌ MAIN ERROR: Notes or Audio required */}
        {errors.main && (
