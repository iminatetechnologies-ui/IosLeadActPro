import {
  ...
  Text,
  View,
  SectionList, // Changed from FlatList to SectionList
  Linking,
  Alert,
  TouchableOpacity,
  StatusBar,
...
const MissedLead = ({navigation}) => {
  const [data, setData] = useState([]);
  const [groupedData, setGroupedData] = useState([]); // Now contains sections
  ...
  // Helper function to format dates and group data
  ...
  // Modified to return sections array for SectionList
  ...
  const fetchData = async (params = {}) => {
    setIsLoading(true);
    try {
      const response = await _get('/getmissed', {params});
  ...
  const handleWhatsappPress = item => {
    console.log('handleWhatsappPress', item);
    const mobile = item?.mobile;
  ...
  const renderLeadItem = ({item}) => {
    ...
    const handleCardPress = () => {
      ...
      } else if (item?.status_id === '1' || item?.status_id === 1) {
  ...
  const visibleLeadItems = getAllVisibleItems();
  const visibleIds = visibleLeadItems.map(item => item.id);
  const areAllVisibleSelected =
    visibleIds.length > 0 && visibleIds.every(id => selectedIds.includes(id));

  // Helper function to limit data for pagination
  const getLimitedSections = () => {
    let currentCount = 0;
  ...
  const limitedSections = getLimitedSections();
  const hasMoreData =
      ...
      {isLoading ? (
        ...
        <>
          ...
          <SectionList
            ...
            stickySectionHeadersEnabled={true} // This enables sticky headers
            contentContainerStyle={styles.list}
            ListEmptyComponent={
              !isLoading && (
