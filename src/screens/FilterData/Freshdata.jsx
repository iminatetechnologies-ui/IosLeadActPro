...
export default function Freshdata({navigation, route}) {
  const statusParam = route?.params?.status;
  const filterParams = route?.params?.filterParams || {};
  // const useridpa = route?.params?.userid;
  const useridpa = route?.params?.userid || filterParams?.userid;
  console.log('statusParam--', statusParam, useridpa);
  ...
  // Fixed fetchData function with proper parameter handling
  const fetchData = useCallback(
    async (status, filters = {}, userid = null) => {
      ...
      try {
        ...
        // Add userid - use the passed userid or fallback to useridpa
        const actualUserId = userid || useridpa;
        if (actualUserId) queryParams.append('userid', actualUserId);
        ...
        console.log('API URL:', `/leads-by-status?${queryParams.toString()}`);

        const response = await _get(
          `/leads-by-status?${queryParams.toString()}`,
        );
        ...
        // Axios error check
        if (error.response) {
          // Server ne error bheja hai (status 400, 500 etc.)
          errorMessage =
  ...
  useFocusEffect(
    useCallback(() => {
      const loadData = async () => {
        ...
        await fetchData(statusParam, params, useridpa);
      };
      loadData();
    }, [statusParam, isFilterApplied, useridpa, fetchData]),
  );
  ...
  const resetFilters = () => {
    ...
    fetchData(statusParam, filterParams, useridpa);
  };
