...
import FilterModal from '../../components/FilterModal analytics';
import {
  widthPercentageToDP as wp,
  heightPercentageToDP as hp,
} from 'react-native-responsive-screen';

import GraphPage from './teamtab/GraphPage';
...
const Team = ({navigation}) => {
  ...
  const fetchChartData = async (params = {}) => {
    try {
      const response = await _get('/getanalyticsdashboard', {params});
  ...
  const applyFilters = async () => {
    try {
      ...
      //  console.log('param0-----------s', params); // ✅ Added for debugging
      const response = await _get('/getanalyticsdashboard', {params}); // ✅ fixed here
        // console.log('after apply filte-----------r ', response);
      const finalData = response?.data?.data;
