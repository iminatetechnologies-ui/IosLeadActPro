...
const StatusTable = ({filters = {}}) => {
  ...
  const fetchTableData = async () => {
    try {
      ...
      const response = await _get(`/callanalytics?${queryParams.toString()}`);
      const users = response?.data?.data?.userWiseCounts;
      ...
      console.error('❌ Error fetching user-wise call data:', error);
      setTableData([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderHeader = () => (
    <View style={[styles.row, styles.headerRow]}>
      ...
      {['Outgoing', 'Incoming', 'Missed', 'Connected'].map((label, index) => (
